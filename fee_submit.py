...
print("the original expected number of blocks of the mining pool in the canonical chain during an epoch:",2400*t/n)
print("the extra expected number of blocks of the mining pool in the canonical chain in above four scenarios during an epoch:",adv_earn)
print("the original expected number of blocks of honest validators in the canonical chain during an epoch:",2400*(n-t)/n)
print("the reduced expected number of blocks of honest validators in the canonical chain in above four scenarios during an epoch:",hon_loss)

fee_original=2400*t/n
fee_extra_risktaking=hon_loss
fee_extra_riskfree=honppe_loss*epoch_ppe
...
fees = x/30 * 2400
plt.xlabel('fraction of adv nodes%')
plt.ylabel('extra fee quantifyed by blocks%')
# analysis results. risk-taking
plt.plot(100*t/n, 100*fee_extra_risktaking/fee_original, label = "overall extra fee")
...
print("simulation:")
for i, j in zip(x/30*100, y/fees):
    print("("+str(i)+","+str(j)+")")
plt.show()